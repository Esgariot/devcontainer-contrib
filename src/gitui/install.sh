...
__step_install_deps() {
  nl install apt-get wget,gpg,sudo,curl #TODO: Instead of 'curl', add featdeps=() array for pre-makedeps
...
__step_pkgver() {
  ...
  _pkgver="$(pkgver)"
  pkgver="${_pkgver}"
  sed -i "${tempsuffix}" -e '1 s/^\s*pkgver=.*/pkgver="'"${_pkgver}"'"/; t' -e '1,// s//pkgver="'"${_pkgver}"'"/' "${1}" 
}
...
install_sh() {
  while :; do
    case "${1-}" in
      --pkgver) __step_pkgver "${script_dir}/PKGBUILD"; exit;;
  ...
  __step_source
  __step_install
  __step_pkgver "${metadir}/PKGBUILD"
  __step_env
  __step_wrapper
}
